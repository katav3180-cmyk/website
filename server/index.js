...
<body>
    ...
    <div class="endpoint">
        ...
        <p>Створює запис. Тіло запиту: <code>{ "id": "string", "auditoryId": "string", "data": "string", "number": "string" }</code>. Власник запису (userId) встановлюється автоматично.</p>
    </div>
...
app.delete("/records/:id", checkRole(['admin', 'teacher']), async (req, res) => {
  try {
    ...
    // Перевірка прав: Адмін може видаляти все, Викладач — тільки свої записи
    if (req.user.role !== 'admin' && record.userId !== req.user.uid) {
      return res.status(403).json({ error: "Доступ заборонено: ви можете видаляти лише власні записи" });
    }
